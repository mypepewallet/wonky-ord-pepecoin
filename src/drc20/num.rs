...
use crate::prc20::params::MAX_DECIMAL_WIDTH;

use super::PRC20Error;

#[derive(PartialEq, PartialOrd, Debug, Clone)]
pub struct Num(BigDecimal);

impl Num {
  pub fn checked_add(&self, other: &Num) -> Result<Self, PRC20Error> {
    Ok(Self(self.0.clone() + &other.0))
  }

  pub fn checked_sub(&self, other: &Num) -> Result<Self, PRC20Error> {
    if self.0 < other.0 {
      return Err(PRC20Error::Overflow {
  ...
  pub fn checked_mul(&self, other: &Num) -> Result<Self, PRC20Error> {
    Ok(Self(self.0.clone() * &other.0))
  }

  pub fn checked_powu(&self, exp: u64) -> Result<Self, PRC20Error> {
    match exp {
  ...
  pub fn checked_to_u8(&self) -> Result<u8, PRC20Error> {
    if !self.0.is_integer() {
      return Err(PRC20Error::InvalidInteger(self.clone().to_string()));
    }
    self.0.clone().to_u8().ok_or(PRC20Error::Overflow {
  ...
  pub fn checked_to_u128(&self) -> Result<u128, PRC20Error> {
    if !self.0.is_integer() {
      return Err(PRC20Error::InvalidInteger(self.clone().to_string()));
    }
    self
      .0
      .to_bigint()
      .ok_or(PRC20Error::InternalError(format!(
        "convert {} to bigint failed",
        self.0
      )))?
      .to_u128()
      .ok_or(PRC20Error::Overflow {
...
impl FromStr for Num {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    if s.starts_with('.') || s.ends_with('.') || s.find(['e', 'E', '+', '-']).is_some() {
      return Err(PRC20Error::InvalidNum(s.to_string()));
    }
    let num = BigDecimal::from_str(s).map_err(|_| PRC20Error::InvalidNum(s.to_string()))?;
