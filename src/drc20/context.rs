...
use crate::prc20::operation::{Action, deserialize_prc20_operation, InscriptionOp, Operation};
use crate::prc20::transfer::Transfer;
use crate::prc20::TransferInfo;
...
impl Message {
  pub(crate) fn resolve<'a, 'db, 'tx>(
    prc20_inscribe_transfer: &'a mut Table<'db, 'tx, &'static InscriptionIdValue, &'static [u8]>,
    ...
    let prc20_operation = match op.action {
      Action::New { inscription: _ } if sat_in_outputs => {
        match deserialize_prc20_operation(
          new_inscriptions
          ...
          Ok(prc20_operation) => prc20_operation,
          _ => return Ok(None),
        }
      }
      // Transfered inscription operation.
      // Attempt to retrieve the `InscribeTransfer` Inscription information from the data store of PRC20.
      Action::Transfer => {
        match get_inscribe_transfer_inscription(prc20_inscribe_transfer, op.inscription_id) {
