...
use {
  ...
  super::*,
  crate::{
    prc20::{script_key::ScriptKey, Tick},
    ...
    templates::{
      ...
      RangeHtml, RareTxt, SatHtml, ShibescriptionJson, TransactionHtml, Utxo, PRC20,
    },
  },
  axum::{
    body,
...
#[derive(Deserialize)]
struct Prc20TickInfoQuery {
  show_holder: Option<bool>,
}

#[derive(Deserialize)]
struct Prc20BalanceQuery {
  show_all: Option<bool>,
...
impl Server {
  pub(crate) fn run(self, options: Options, index: Arc<Index>, handle: Handle) -> SubcommandResult {
    Runtime::new()?.block_on(async {
      ...
      let router = Router::new()
        ...
        .route("/prc20/tick/:tick", get(Self::prc20_tick_info))
        .route("/prc20/tick", get(Self::prc20_all_tick_info))
        .route(
            "/prc20/balance/:address",
            get(Self::prc20_by_address_unpaginated),
        )
  ...
  async fn prc20_by_address(
    Extension(index): Extension<Arc<Index>>,
    Path(params): Path<(String, u32)>,
    Query(query): Query<Prc20BalanceQuery>,
  ) -> ServerResult<Response> {
    Self::get_prc20_by_address(index, params.0, Some(params.1), query).await
  }

  async fn prc20_by_address_unpaginated(
    Extension(index): Extension<Arc<Index>>,
    Path(params): Path<String>,
    Query(query): Query<Prc20BalanceQuery>,
  ) -> ServerResult<Response> {
    Self::get_prc20_by_address(index, params, None, query).await
  }

  async fn get_prc20_by_address(
    index: Arc<Index>,
    address: String,
    page: Option<u32>,
    ...
    task::block_in_place(|| {
      ...
      let mut prc20_utxos: Vec<(PRC20, Utxo, InscriptionId, u64, u64, bool)> = Vec::new();
      ...
      if show_utxos {
        ...
        for outpoint in outpoints {
          ...
          for inscription_id in inscriptions {
            ...
            if let Some(content) = str_content {
              let prc20 = PRC20::from_json_string(content.as_str());
              if let Some(prc20) = prc20 {
                if let Some(filter) = query.tick.clone() {
                  if filter != prc20.clone().tick.unwrap_or_default() {
                    continue;
                  }
                }
                let txid = outpoint.txid;
        ...
        if !inscription_ids_to_check.is_empty() {
          ...
          for (prc20, _, id, _, _, ref mut valid) in &mut prc20_utxos {
            if let Some(ref op) = prc20.op {
              if *op == Operation::Transfer {
      ...
      let mut prc20balances: Vec<PRC20Balance> = Vec::new();

      let balance = index
        .get_prc20_balances(&ScriptKey::from_address(
          address_from_str,
          index.get_network()?,
        ))
        .map_err(|err| ServerError::BadRequest(err.to_string()))?;
      ...
      if !balance.is_empty() {
        for entry in balance {
          ...
          let utxos: Vec<PRC20Output> = prc20_utxos
            .iter()
            ...
            .collect();
          let token_info = index.get_prc20_token_info(&entry.tick.clone())?;
          ...
          if let Some(prc20_balance) = PRC20Balance::from_strings(
            tick.as_str(),
            ...
            prc20balances.push(prc20_balance);
          }
        }
      }
      Ok(Json(json!({"prc20": prc20balances})).into_response())
    })
  }
  ...
  async fn prc20_tick_info(
    Extension(index): Extension<Arc<Index>>,
    Path(tick): Path<String>,
    Query(query): Query<Prc20TickInfoQuery>,
    ...
    if query.show_holder.unwrap_or(false) {
      let extended_token_info: Vec<ExtendedTokenInfo> = token_info
        ...
        .map(|info| {
          ...
          let holder = index
            .get_prc20_token_holder(&tick.clone())
            .unwrap_or(Vec::new());
  ...
  async fn install_script() -> Redirect {
    Redirect::to("https://raw.githubusercontent.com/apezord/ord-pepecoin/master/install.sh")
  }
...
mod tests {
  ...
  fn install_sh_redirects_to_github() {
    TestServer::new().assert_redirect(
      "/install.sh",
      "https://raw.githubusercontent.com/apezord/ord-pepecoin/master/install.sh",
    );
  }
