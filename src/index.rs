...
use crate::prc20::{Balance, max_script_tick_key, min_script_tick_key, script_tick_key, Tick, TokenInfo, TransferableLog, min_script_tick_id_key, max_script_tick_id_key};
...
define_table! { PRC20_BALANCES, &str, &[u8] }
define_table! { PRC20_TOKEN, &str, &[u8] }
define_table! { PRC20_INSCRIBE_TRANSFER, &InscriptionIdValue, &[u8] }
define_table! { PRC20_TRANSFERABLELOG, &str, &[u8] }
define_multimap_table! { PRC20_TOKEN_HOLDER, &str, &str}
...
impl Index {
    ...
    let database = match unsafe { Database::builder().open(&path) } {
      Ok(database) => {
        {
          ...
          index_transactions = statistics
            ...
            .value()
            != 0;
          index_prc20 = statistics
            .get(&Statistic::IndexPrc20.key())?
            .unwrap()
            .value()
            != 0;
        }
        ...
        if error.kind() == io::ErrorKind::NotFound =>
      {
...

        {
          ...
          index_prc20 = options.index_dunes();
          index_dunes = options.index_dunes();
          index_sats = options.index_sats;
          index_transactions = options.index_transactions;

          statistics.insert(&Statistic::IndexPrc20.key(), &u64::from(index_prc20))?;
  ...
  pub(crate) fn get_prc20_balances(&self, script_key: &ScriptKey) -> Result<Vec<Balance>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let prc20_token_balance = rtx.open_table(PRC20_BALANCES)?;
  ...
  pub(crate) fn get_prc20_balance(
    &self,
  ...
  pub(crate) fn get_prc20_token_info(&self, tick: &Tick) -> Result<Option<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;

      let prc20_token_info = rtx.open_table(PRC20_TOKEN)?;
      return Ok(
        prc20_token_info
  ...
  pub(crate) fn get_prc20_tokens_info(&self) -> Result<Vec<TokenInfo>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;
  ...
  pub(crate) fn get_prc20_token_holder(&self, tick: &Tick) -> Result<Vec<ScriptKey>> {
    if self.block_count().unwrap() >= self.first_inscription_height {
      let rtx = self.database.begin_read()?;
  ...
  pub(crate) fn get_prc20_transferable_by_range(
    ...
    let rtx = self.database.begin_read()?;
    let prc20_transferable_log = rtx.open_table(PRC20_TRANSFERABLELOG)?;
    let result = Ok(
  ...
  pub(crate) fn get_prc20_transferable_by_tick(
    &self,
    script: &ScriptKey,
    tick: &Tick,
  ) -> Result<Vec<TransferableLog>, redb::Error> {
  ...
  pub(crate) fn get_prc20_transferable_by_id(
    &self,
    script_key: &ScriptKey,
    ...
    if self.block_count().unwrap() >= self.first_inscription_height {
      ...
      let transferable_log_vec: Vec<TransferableLog> = prc20_transferable_log
